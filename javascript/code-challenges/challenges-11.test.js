...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let dubFilter = filteredArray.map(array => array.filter(number => typeof(number) === 'number'));

  let newArray = [];
  for (let i = 0; i < dubFilter.length; i++){
    newArray.push([]);
    for (let j = 0; j <= dubFilter.length; j++){
      if (dubFilter[i][j]) {
        newArray[i].push(Math.pow(2, dubFilter[i][j]));
      }
    }
  }
  return newArray;
