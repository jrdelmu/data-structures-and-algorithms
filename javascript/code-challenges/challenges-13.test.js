...
// Solution code here...
var longStr;
arr.reduce((val, current) => val.length > current.length ? longStr = val : longStr = current,'');
...
const findHappiness = (arr) => {
  // Solution code here...
  const smileyArr =[];
  arr.forEach( str => str.includes(":)") ? smileyArr.push(str) : smileyArr);
  return smileyArr;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let oddIdxArr =[];
  str.split('').reduce( (val, current, i) => i % 2 === 1 ? oddIdxArr.push(current) : oddIdxArr, []);
  return oddIdxArr.join('');
};
