...
const typeNum = (arr) => {
  ...
  let numOnly = function(value) {
    if(typeof(value) === 'number'){
      return value;
    }
  }
let nums = arr.filter(numOnly)
...
const containsAnd = (arr) => {
  // Solution code here...
  let andOnly = function(string){
    if(string.includes('and') === true){
      return string;
    }
  }
  let strings = arr.filter(andOnly)
...
const oddValues = (arr) => {
  // Solution code here...
  let oddOnly = function(value){
    if(value % 2 !== 0){
      return value;
    }
  }

  let nums = arr.filter(oddOnly)
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  let arrTwo = function(value){
    for(let i = 0; i < arr.length; i++){
      if(value[i] !==forbiddenValues)
        return value[i]
    }
  }

  let nums = arr.filter(arrTwo)
